...
- reads your **group notifications**, opens each new post's permalink, **waits for the body to finish rendering** (Facebook streams post text in progressively — reading too early grabbed a ~95-char preview and the parser got nothing), then captures the full text, **and**
- **sweeps** one target group's chronological feed per cycle (round-robin), scrolling until it re-sees **several consecutive** already-captured posts (so a pinned post can't short-circuit it) — so posts Facebook never notified you about (video/reel posts especially) are still caught.

Captured posts run the normal parse → dedup → score → WhatsApp/digest pipeline with group/author metadata. Setup: for each apartment group, set **Notifications → All posts** (helps the notification path; the sweep is the safety net for what it still misses). The reader role is latched in **sessionStorage** — Facebook periodically prunes the userscript's *localStorage* (which used to wipe the reader's lease mid-session and freeze the tab after a few posts; sessionStorage survives that), with the localStorage lease kept only as a restart-recovery hint. A companion **watchdog** auto-revives the tab if the reader goes silent 45+ min, and the script **auto-updates** via Tampermonkey (`@updateURL` → this repo, so no manual reinstall).
...
- **External dead-man's switch** — the scheduler pings `HEALTHCHECK_URL` (e.g. a free [healthchecks.io](https://healthchecks.io) check) **only when both sensors are fresh**, so the external service alarms you — over a channel independent of Twilio *and* the Mac — on **coverage** loss, not just scheduler-process death. No-op until you set the env var (it's in `.env.example`).
- **Alive-but-blind flag** — the daily heartbeat also warns when a sensor is heartbeating yet captured **0 listings in 24h** — the signature of selector/DOM rot that otherwise stays invisibly green.
- **Delivery guard** — a console fallback no longer counts as "delivered" when you intend WhatsApp (Twilio configured), so a `.env` typo or an expired Twilio-sandbox session can't silently mark every match sent-and-suppressed; alert bodies are length-capped so an over-long post can't fail Twilio (21617) forever.
...
- **Self-healing** — when the Facebook reader goes 45+ min stale during waking hours, `fb-watchdog` first **navigates the existing Facebook tab** back to `notifications#re-agent` (reusing the tab, *not* opening a duplicate — two readers would contend for the role and ping-pong) and only WhatsApps you if that didn't bring it back next cycle (i.e. a real CAPTCHA). So a Mac-woke / tab-dropped outage recovers unattended.
